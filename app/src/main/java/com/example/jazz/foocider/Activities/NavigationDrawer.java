...
public class NavigationDrawer extends AppCompatActivity
    ...
    public boolean onNavigationItemSelected(MenuItem item) {
        ...
        if (id == R.id.nav_options) {
            Intent intent = new Intent(this, ViewOptionsActivity.class);
    ...
    public Suggestion getSuggestion() {
        ...
        for (int i = 0; i < size; i++){
            double score = (random.nextInt(size) * suggestionList.get(i).getPreference()) - (0.2 * suggestionList.get(i).getVisted());
            if (score >= max){
                max = score;
                index = i;
            }
